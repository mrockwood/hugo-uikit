...
export const Slidenav = {
    ...
    connected() {
        addClass(this.$el, 'c-slidenav');
    },

    computed: {

        icon({icon}, $el) {
            return hasClass($el, 'c-slidenav-large')
                ? `${icon}-large`
                : icon;
        }
...
export const Search = {
    ...
    computed: {
        ...
        icon({icon}, $el) {
            return hasClass($el, 'c-search-icon') && parents($el, '.c-search-large').length
                ? 'search-large'
                : parents($el, '.c-search-navbar').length
                    ? 'search-navbar'
                    : icon;
        }
...
export const Close = {
    ...
    computed: {
        ...
        icon() {
            return `close-${hasClass(this.$el, 'c-close-large') ? 'large' : 'icon'}`;
        }
